...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (slug === 'upload' && req.method === 'POST') {
      ...
      if (!data || !filename) return err(res, 'Нет данных файла');
    ...
    if (slug === 'me' && method === 'GET') {
      return json(res, { email: user.email });
    }

    if (slug === 'dashboard' && method === 'GET') {
    ...
    if (slug === 'inquiries' && method === 'GET') {
      const rows = await pool.query('SELECT * FROM inquiries ORDER BY created_at DESC');
      return json(res, rows.rows);
    }

    const inqMatch = slug.match(/^inquiries\/(\d+)$/);
    ...
    for (const [table, config] of Object.entries(crudTables)) {
      ...
      const idMatch = slug.match(new RegExp(`^${table}\\/(\\d+)$`));
      if (idMatch) {
